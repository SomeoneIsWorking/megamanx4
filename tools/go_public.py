...
Checks:
  ...
  C. gitignore  — tracked text that references a *sensitive* gitignored item
                  (regenerable ignores like build/ or scratch/ are NOT flagged)
...
COPYRIGHT_PATTERNS = [
    ...
    r"(^|/)rom\.rvz$", r"(^|/)rom\.iso$",
    # PSX disc images and the boot executable extracted from one. These are the shapes that matter in
    # THIS repo: the disc is Capcom's, and so is SLUS_005.61. They are PATH patterns (check A), which
    # is why naming the executable here is safe while naming it in .gitignore is not — check C turns
    # every gitignore pattern into a TEXT search token, and every doc in this repo names SLUS_005.61
    # in prose.
    r"\.chd$", r"\.cue$", r"\.ecm$", r"\.pbp$", r"\.m3u$",
    r"(^|/)SLUS_005\.61$", r"(^|/)SLUS_00561$",
]
...
#      • regenerable output — build/, scratch/ dumps, *.o, logs …
...
BENIGN_IGNORE = [
    ...
    r"^build", r"^port/build", r"^scratch", r"\.o$", r"\.so$",
