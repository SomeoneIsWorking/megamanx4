...
def on_disk(index, token):
    """Where a token's file actually IS, ignoring git. Resolution is TRACKED-file based, so the two
    answers differ in exactly one situation that matters: a repo with no commits yet."""
    for repo in index.repos:
        p = os.path.join(repo, token)
        if os.path.exists(p):
            return os.path.relpath(p, index.root)
    return None


def why_unresolved(unresolved, index):
    """Say WHICH of the two it is. Saying "this repo does not have <file>" about a file sitting on
    disk sends the reader hunting a missing file that is not missing; the real cause, in a tree whose
    first commit has not landed, is that resolution indexes `git ls-files` and nothing is tracked."""
    if not unresolved:
        return "evidence names no source file at all"
    toks = sorted(set(unresolved))
    untracked = [t for t in toks if on_disk(index, t)]
    absent = [t for t in toks if t not in untracked]
    parts = []
    no_commits = git(index.root, "rev-parse", "--verify", "HEAD")[0] != 0
    if untracked:
        parts.append("evidence names files that EXIST on disk but are NOT TRACKED IN GIT"
                     + (" (this repo has NO COMMITS yet)" if no_commits else "")
                     + " — staleness is a git query, so re-run after they are committed: "
                     + ", ".join(untracked[:4]))
    if absent:
        parts.append("evidence names files not found on disk under "
                     + ", ".join(os.path.relpath(r, index.root) or "." for r in index.repos)
                     + ": " + ", ".join(absent[:4]))
    return "; ".join(parts)
...
def analyse_claims(root=None, use_cache=True):
    ...
    for e in entries(claims_dir):
        ...
        if not deps:
            row["kind"] = "blind"
            row["why_blind"] = why_unresolved(unresolved, index)
            continue
        if base is None:
            row["kind"] = "blind"
...
def cmd_brief(a):
    ...
    # FIXED HERE (the workspace's recorded "invocation trap"): this list named ONLY
    # `docs/code-map.md`, the hyphenated Tomba2Engine spelling. spyro, spider1, vagrant and THIS repo
    # use `docs/codemap.md`, so the CODEMAP section could never fire and a brief silently consulted one
    # registry fewer than it claimed to. Both spellings are now tried; the first that exists wins.
    # Also `--untracked`: `git grep` without it sees only the INDEX, so in a repo whose docs are not yet
    # committed every section here returned nothing — again silently.
    for name, cands in (("CODEMAP", ("docs/codemap.md", "docs/code-map.md")),
                        ("RE-FRONTIER", ("docs/re-frontier.md",))):
        data = next((c for c in cands if os.path.exists(os.path.join(ROOT, c))), None)
        if data:
            hits = _run(["git", "grep", "-in", "--untracked", "-m", "3", q, "--", data])
            if hits:
                print(f"\n  {name}")
    ...
    if tc or ti:
        ...
        print("\n  THE CLAIMS AND INSTRUMENTS LEDGERS DO NOT EXIST IN THIS "
              ...
              "actually consulted.")
        # FIXED HERE: this branch used to fall out with exit 0, i.e. "searched NOTHING" was
        # indistinguishable, to anything mechanical, from "searched everything and found nothing".
        # The paragraph above was already honest to a human; the exit code was not.
        return 2
    else:
