#!/usr/bin/env bash
# run.sh — the USER's build-and-play entry point for the Mega Man X4 native PC port.
#
#   ./run.sh ["/path/to/Mega Man X4 (USA).chd"]
#
...
# sync submodules, resolve the disc, extract and identity-check SLUS_005.61, build what builds — and
# then refuses at the recompile step, because emit.py needs this game's seed set and this port has no
# boot group (docs/re-frontier.md RE-01/RE-02).
set -eu
cd "$(dirname "$0")"
...
# ---- 0b. sync git submodules (this repo's own: external/mmx4 reference; the framework's nested
# vendors are the SHARED clone's concern now, initialized once in the dev clone) -------------
...
# external/mmx4 is a REFERENCE and is deliberately not required to build, so a missing one is a
# warning, not a failure. (The framework's own nested vendors are the SHARED clone's — they were
# initialized once there, and this repo's sync must not walk into them and disturb another port.)
if command -v git >/dev/null && [ -f .gitmodules ] && [ -f external/psxport/scripts/sync-submodules.sh ]; then
  [ -d external/mmx4/config ] || say "note: external/mmx4 (the AGPL reference decomp) is not checked \
out — the identity check will say it could not run. \`git submodule update --init external/mmx4\`"
  bash external/psxport/scripts/sync-submodules.sh || die "submodule sync failed"
fi

# ---- 1. resolve the disc + extract the boot executable ------------------------------------------
# ONE implementation of the resolution order (CLI arg > $PSXPORT_X4_DISC > .env > *.chd drop-in):
...
cmake --build build -j "$JOBS" --target megamanx4_seam || die "seam check failed"
