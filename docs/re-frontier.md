...
at libetc's exact counter-wait helper and its one-field frame commit. RE-04 now owns the separate CD
...
than the archive loader: retail task entry 0x8001D064 is created but shared psxport's BIOS ChangeThread
is a no-op, so the front-end initializer that leads to the first archive/read command never runs. The
former FPS60 overflow is resolved as a missing frame commit, not a malformed OT or CD failure.
...
- status: re-partial
- deps: RE-01
- evidence: Retail SLUS_005.61 (SHA-1 213733031136d095ca275d6957695aa25011cfa5): tools/verify_cd_irq.py checks six contract groups. InitPAD builds priority-1 element 0x8013BBF8 with handler 0x800EE274 and verifier 0x800EE2DC; the verifier returns non-zero only for IRQ0. startIntr installs jmp_buf 0x8011CBCC whose live saved RA is 0x800E5194; that continuation calls trapIntr 0x800E5208, whose eleven-slot table starts at 0x8011CB98. CD_init registers 0x800E7944 in slot 2 and that callback drains ready/sync results through 0x8013BA80/0x8013BA78 and updates libcd status. Selftest is 6/6. Exact framework pin 3418a79b untraced CDC evidence reaches Getstat/Init/Demute/Setmode with every IRQ2 acknowledged. scratch/logs/re04-3418-thread-boundary-trace.log then records task creation with entry 0x8001D064 and 86 ChangeThread calls from the retail scheduler, while 0x8001D064 and its first callee remain never-called; shared runtime/recomp/threads.cpp implements that B0:0x10 service as a no-op. No synchronous-success override, direct entry dispatch, or polling bypass exists.
- where: psxport.pin; game/recomp_seeds.json; tools/verify_cd_irq.py; game/core/vsync_sync.cpp; scratch/logs/re04-3418-frame-commit-runtime.log and scratch/logs/re04-3418-thread-boundary-trace.log (untracked runtime evidence); shared psxport runtime/recomp/bios_interrupt.cpp, hle.cpp, and threads.cpp
- gap: The IRQ/callback/result-state boundary and guest frame fence are complete. The next required mechanism is shared BIOS cooperative thread scheduling: OpenThread must preserve the retail entry/sp/gp context and ChangeThread must transfer into 0x8001D064 before the archive loader can be observed. The loader's destination/streaming contract remains downstream and must not be guessed from the archive sizes. A fresh true-oracle state capture is unavailable because no true-oracle executable is installed; the upstream Mednafen PC-coverage set is corroboration only and does not establish order or result state.
- notes: This separates enhancement job A from job B. Raw CD IRQ delivery is faithful and shared; do not replace it with synchronous CdInit success. Direct-calling 0x8001D064 game-locally would bypass the very thread contract under investigation. Coordinate the shared execution-model change and cross-consumer tests before editing psxport. Issue #10 records the separate frame-fence cause and fix.
...
- gap: VBlank synchronization and the frame fence are verified. The live stop is RE-04's separate BIOS-thread handoff; every other unmeasured platform-HLE entry remains zero.
