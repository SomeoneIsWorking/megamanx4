...
live boundary is now the retail task handoff: task entry `0x8001D064` is created but never runs because
the shared BIOS thread model does not implement `ChangeThread`. The in-flight local task bypass does
not cross that boundary either. The executable now defaults to the only honest shipping picture for
...
| | |
|---|---|
| `external/psxport/` | the PSX-generic framework (shared checkout; verified pin `3418a79b`): MIPS→C recompiler, runtime substrate, GTE/SPU/MDEC/CD/GPU backends, SDK HLE, SBS differential harness, SDL_GPU renderer. Framework fixes land in the workspace dev clone, never as per-game copies. |
...
| Framework seam — measured legacy facts | `game/core/game_config.cpp`, `game/core/legacy_game_interface.h` | 🟡 | Explicit compatibility debt behind `X4Runtime`, not the title API. RE-01's crt0 group and RE-06's pad buffers are measured and wired; both shipping bindings are independently diffed against the retail executable. RE-11 declares one exact platform window, [0x800E4EF8,0x800E4F94), for the measured VBlank counter-wait helper. Generic synchronous-CD HLE entries remain zero because the retail route is verified. Next: delete groups as psxport adds narrow typed fact interfaces. |
...
| Inherited title runtime | `game/core/x4_runtime.{h,cpp}` → `x4::X4Runtime` | ✅ | First real `GameRuntime` consumer. The process-lifetime derived object owns render-path policy, measured guest-main dispatch, and RE-11 override installation. X4 deliberately has no native producers, so it defaults to `gte`, permits diagnostic `psx`, and refuses `native`. It derives the bounded adapter only for measured compatibility facts. |
...
| VBlank synchronization | `game/core/vsync_sync.{h,cpp}`, `tools/verify_vsync.py` | ✅ | Retail bytes prove VSync's two calls to exact helper [0x800E4EF8,0x800E4F94), counter 0x8011DC50, IRQ-0 handler 0x800E56FC, and its eight-slot callback table at 0x8011DC30. Shipping code owns only the helper, runs the retail handler, and commits exactly one field through the authoritative frame fence. Verifier is 6 groups / selftest 5/5; a 20-second untraced run presents continuously with no watchdog or capture overflow. |
| Process entry | `game/core/main.cpp` | 🟡 | Installs one process-lifetime `X4Runtime`, executes framework bring-up and native CRT0, then dispatches guest `gameMain` through the derived owner. CD initialization and result IRQs complete, but shared BIOS thread scheduling remains the boundary: retail task entry 0x8001D064 is created and never runs, so no archive/read command is reached. |
...
| Focused C++ tests | `tests/test_x4_runtime.cpp` | ✅ | Hermetically proves the installed authority is derived, Core snapshots it, measured compatibility views remain available, boot/override callbacks have left `GameHooks`, and the title still defaults to `gte`, permits diagnostic `psx`, and rejects `native`. CTest builds it with Clang against the shipping runtime. |
...
| Gate on a built binary | — | 🟡 | A bounded headless Vulkan diagnostic proves native CRT0 → InitHeap → guest `gameMain` and announces `gte` as the picture path. An explicit `native` selection refuses with exit 2. The same run reaches func_80012600 40,431 times while task entry 0x8001D064 remains unreachable, so the current picture is still black for an upstream scheduling reason. |
| Everything about the game itself | — | 🟡 | Boot layout, both InitPAD buffers, VBlank wait/IRQ/frame fence, CD interrupt/callback/result state, required guest render path, and the complete GTE projection-writer census are measured. The retail boot-thread handoff remains unsupported by shared psxport; the archive loader, player-object system, and wait-state timers remain downstream. |
