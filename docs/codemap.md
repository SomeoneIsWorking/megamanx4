...
| Process entry | `game/core/main.cpp` | 🟡 | Executes framework bring-up and native CRT0, then dispatches guest `gameMain`. The current runtime frontier is the stock CD-init IRQ2 retry loop. |
...
| Gate on a built binary | — | 🟡 | A bounded software-Vulkan diagnostic proves native CRT0 → InitHeap → guest `gameMain`, installs the exact VBlank helper route, then shows repeated CD commands 0x01/0x0A and unclaimed IRQ2. A reusable advancement/end-state gate is still absent and is required before claiming a boot milestone beyond that exact trace. |
| Everything about the game itself | — | 🟡 | Boot layout, both InitPAD buffers, and the static VBlank wait/IRQ contract are measured. CD/HLE now has an independently classified boot chain and an unclaimed IRQ2 boundary; its interrupt element/result contract is still unknown. The player-object system and wait-state timers are not started. Widescreen has one static source lead — boot `func_8001213C` sets GTE OFX/OFY/H to 160/120/512 — but its later write census and runtime verification are absent. |
