...
| | |
|---|---|
| `external/psxport/` | the PSX-generic framework (shared checkout; recorded pin `9c2e3f1c`): MIPS→C recompiler, runtime substrate, GTE/SPU/MDEC/CD/GPU backends, SDK HLE, SBS differential harness, SDL_GPU renderer. Framework fixes land in the workspace dev clone, never as per-game copies. |
