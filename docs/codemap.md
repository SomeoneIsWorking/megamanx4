...
**This repo is scaffolding. The port does not run; the crt0 group and two pad buffers are reverse-engineered.**
...
| | |
|---|---|
| `external/psxport/` | the PSX-generic framework (shared checkout; verified pin `be381503`): MIPS→C recompiler, runtime substrate, GTE/SPU/MDEC/CD/GPU backends, SDK HLE, SBS differential harness, SDL_GPU renderer. Framework fixes land in the workspace dev clone, never as per-game copies. |
...
| crt0 / boot-group derivation | `tools/verify_crt0.py` | ✅ | Derives the complete group from the retail entry function and compares 19 shipping facts without a second expectation table. The framework-source verdict locates 15/15 mechanisms. `--selftest` is 25/25 locally and 28/28 with the independent Tomba!2 control, including binary/config sabotage, removal of four framework mechanisms, and malformed-input refusals. Blind spot: static immediate dataflow, not a runtime boot. |
...
| Framework seam — recomp registry | `game/core/recomp_register.cpp` | ⬜ | Deliberately UNWRITTEN. Its no-substrate body is included in `megamanx4_seam` so it stays compile- and lint-covered; it names generated symbols only under `MMX4_HAVE_SUBSTRATE`, where it `#error`s until the real registry is written. |
...
| Build and C++ verifier | `CMakeLists.txt`, `cmake/megamanx4_port.cmake`, `external/psxport/tools/check_cpp_style.py` | 🟡 | The Clang build emits `build/compile_commands.json`; `megamanx4_seam` compiles every first-party TU. `ctest --test-dir build --output-on-failure` calls psxport's shared checker with this repo as `--root`: tracked first-party C/C++ is format-checked, capped at 1,200 lines, and clang-tidied through the real compile database. `external/` and `generated/` are excluded; there is no copied checker or pre-commit hook. The port binary remains unavailable until the substrate exists. |
| Play launcher | `run.sh`, `tools/run.py`, `tools/test_run.py` | 🟡 | `run.sh` is a two-line stable exec wrapper. Python owns toolchain/framework checks, provisioning, build/CTest orchestration, and the honest exit-3 RE-02 refusal. The five-case launcher test exercises the shipping function through an injected host seam: full positive flow, empty-value shell defaults, missing-tool refusal, provisioning refusal, and the trivial executable wrapper contract. Agents must not invoke the real launcher. |
