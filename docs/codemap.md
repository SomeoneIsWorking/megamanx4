...
| | |
|---|---|
| `external/psxport/` | the PSX-generic framework (shared checkout; verified pin `7f5d3f13`): MIPS→C recompiler, runtime substrate, GTE/SPU/MDEC/CD/GPU backends, SDK HLE, SBS differential harness, SDL_GPU renderer. Framework fixes land in the workspace dev clone, never as per-game copies. |
