...
| | |
|---|---|
| `external/psxport/` | the PSX-generic framework (shared checkout; verified pin `9f1bb927`): MIPS→C recompiler, runtime substrate, GTE/SPU/MDEC/CD/GPU backends, SDK HLE, SBS differential harness, SDL_GPU renderer. Framework fixes land in the workspace dev clone, never as per-game copies. |
...
| VBlank synchronization | `game/core/vsync_sync.{h,cpp}`, `tools/verify_vsync.py` | 🟡 | Retail bytes prove VSync's two calls to exact helper [0x800E4EF8,0x800E4F94), counter 0x8011DC50, IRQ-0 handler 0x800E56FC, and its eight-slot callback table at 0x8011DC30. Shipping code owns only the helper and runs the retail handler; verifier is 6 groups / selftest 4/4. 🟡 because boot only uses VSync(-1), so the blocking route is installed but not runtime-exercised. |
| Process entry | `game/core/main.cpp` | 🟡 | Executes framework bring-up and native CRT0, then dispatches guest `gameMain`. The former CD-init retry boundary is cleared through retail IRQ delivery. A 20-second run against pinned framework `9f1bb927` has no watchdog or capture overflow; the archive loader/open/read/decode contract remains unclassified. |
...
| Gate on a built binary | — | 🟡 | A bounded software-Vulkan diagnostic against pinned framework `9f1bb927` proves native CRT0 → InitHeap → guest `gameMain` → HookEntryInt `0x800E5194` → trapIntr slot 2 → CD callback `0x800E7944`, including I_STAT acknowledgement, sync-result callback, five clean callback returns, and advancement beyond CdInit. It runs to the external 20-second bound with no watchdog or capture overflow. A reusable archive-loader/end-state gate is still absent. |
| Everything about the game itself | — | 🟡 | Boot layout, both InitPAD buffers, static VBlank wait/IRQ, and CD interrupt/callback/result state are measured. The archive loader, player-object system, and wait-state timers remain unclassified. Widescreen has one static source lead — boot `func_8001213C` sets GTE OFX/OFY/H to 160/120/512 — but its later write census and runtime verification are absent. |
