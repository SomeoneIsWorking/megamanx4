...
RE-11 now has a retail-derived VBlank route at libetc's exact wait helper, but boot only queries
`VSync(-1)` and never reaches it. Focused runtime evidence moves the live stall to RE-04's unclaimed
CD IRQ2 path. The three enhancements still have no implementation.
...
| | |
|---|---|
| `external/psxport/` | the PSX-generic framework (shared checkout; verified pin `ce2c83ad`): MIPS→C recompiler, runtime substrate, GTE/SPU/MDEC/CD/GPU backends, SDK HLE, SBS differential harness, SDL_GPU renderer. Framework fixes land in the workspace dev clone, never as per-game copies. |
...
| Framework seam — config | `game/core/game_config.cpp` | 🟡 | Compiles. RE-01's crt0 group and RE-06's pad buffers are measured and wired; both shipping bindings are independently diffed against the retail executable. RE-11 declares one exact platform window, [0x800E4EF8,0x800E4F94), for the measured VBlank counter-wait helper; every other HLE entry remains zero. |
...
| Process entry | `game/core/main.cpp` | 🟡 | Executes framework bring-up and native CRT0, then dispatches guest `gameMain`. CD initialization and result IRQs complete, and the guest main loop presents at its measured field boundary. The next stop is shared BIOS thread scheduling: retail task entry 0x8001D064 is created, but psxport's B0:0x10 ChangeThread is a no-op, so no archive/read command is reached. |
...
| Gate on a built binary | — | 🟡 | A bounded software-Vulkan diagnostic proves native CRT0 → InitHeap → guest `gameMain` → HookEntryInt `0x800E5194` → trapIntr slot 2 → CD callback `0x800E7944`, including I_STAT acknowledgement and sync-result callback. After the one-field frame-fence fix it presents for 20 seconds with no watchdog or capture overflow. Against exact framework pin `ce2c83ad`, a focused trace proves the next boundary by showing 140 ChangeThread calls while task entry 0x8001D064 remains unreachable. |
| Everything about the game itself | — | 🟡 | Boot layout, both InitPAD buffers, VBlank wait/IRQ/frame fence, and CD interrupt/callback/result state are measured. The retail boot-thread handoff is now located but unsupported by shared psxport; the archive loader, player-object system, and wait-state timers remain downstream. Widescreen has one static source lead — boot `func_8001213C` sets GTE OFX/OFY/H to 160/120/512 — but its later write census and runtime verification are absent. |
