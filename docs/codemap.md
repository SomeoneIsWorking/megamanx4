...
| Framework seam — config | `game/core/game_config.cpp` | 🟡 | Compiles. The crt0 constants/binding remain covered by `tools/verify_crt0.py` and RE-01's named framework-integration gap. **RE-06 is complete:** `tools/verify_pad.py` independently diffs the named pad constants and actual struct bindings against retail InitPAD dataflow. `padDriverFn` and the pointer-table fields stay zero deliberately because the executable passes direct buffers and psxport owns a fixed-buffer fallback. Every unmeasured address group remains `0` with its frontier step named. |
