...
* The retired shell sync helper did not fix it either: it printed *"checked 2 of 2
...
## Fixes landed
README "Getting started" and CLAUDE.md's build gate both carried the two-line form and stated explicitly
that recursive initialization and the retired helper did not suffice, citing the KNOWN-DEFECT doc.
...
The framework's current `scripts/sync_submodules.py` also refuses to certify a partial nested
inventory, so the false-success behavior described above is no longer live.
...
INTO the read-only pinned dependency. It now prefers an existing binary and otherwise builds into this
repo's gitignored `build/tools/discdump-<hash of the checkout>/` — keyed by checkout, so pointing
